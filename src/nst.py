...
def neural_style_transfer(content_image_path, style_image_path, output_image_path, iterations=1000, content_weight=1e3, style_weight=1e-2):
    try: 
        ...
        target_shape = tf.maximum(tf.shape(content_image)[1:3], tf.shape(style_image)[1:3])
        content_image = tf.image.resize(content_image, target_shape)
        style_image = tf.image.resize(style_image, target_shape)
        ...
        for i in range(iterations):
            ...
            if i % 10 == 0:
                logger.info(f'Iteration: {i}, Loss: {loss}, Style Loss: {style_score}, Content Loss: {content_score}')
        ...
        logger.error(f"Error in neural_style_transfer: {str(e)}")
        raise

def perform_nst(content_image_path, style_image_path, output_image_path):
    try:
        for i in range(iterations):
            neural_style_transfer(content_image_path, style_image_path, output_image_path, iterations=1)
...
def perform_nst_with_progress(content_image_path, style_image_path, output_image_path, iterations=100):
    try:
        for i in range(iterations):
            neural_style_transfer(content_image_path, style_image_path, output_image_path, iterations=1)
            progress = (i + 1) / iterations * 100
